...
export default function Page() {
  return <div className='min-h-screen' style={{ backgroundImage: `url("/studio/tests/bg-main2.svg")`, backgroundPosition: 'top center', backgroundSize: 'cover' }}>
    <SDK1 />
  </div>
}
  ...
  return (
    <div className="pt-20 pb-24">
      <div className="flex flex-col justify-center items-cente max-w-6xl px-4 mx-auto">
        <div className="grid gap-4 mt-8 max-w-lg mx-auto">
          <div className="bg-[#161616] flex backdrop-blur-sm bg-opacity-5 overflow-hidden rounded-xl border border-white border-opacity-5 h-max">{features.map((el, index) => {
            return <div onClick={() => {
              setSelected(index)
            }} key={index} className={`p-3 cursor-pointer transition-all hover:bg-opacity-5 border-opacity-5 last:border-none bg-white bg-opacity-0 ${selected === index ? "bg-opacity-5" : ""}`}>
              <div className="flex gap-4">
                <img src={el.icon} className="w-6 self-start" />
                <div>
                  <h3 className="font-medium text-white text-xs">{el.title}</h3>
                </div>
              </div>
            </div>
          })}
          ...
          <div className="relative">
            <div className="relative group" style={{ maxWidth: `calc(100vw - 46px)` }}>
              <div className={`no-scrollbar border overflow-x-auto w-full bg-opacity-5 backdrop-blur-sm pb-4 text-sm border-white border-opacity-10 p-3 z-10 rounded-md`} style={{
